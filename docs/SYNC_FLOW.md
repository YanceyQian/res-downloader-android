...
每次同步时，在原有代码同步检查清单基础上，增加以下配置相关检查：
...
| 同步类型 | 触发条件 | 更新方式 | 用户感知 |
|---------|---------|---------|---------|
| **代码同步** | 原项目有 Go 代码变更 | 发布新 APK | 需要用户更新应用 |
| **配置同步** | 平台 API 变更 | 应用自动获取 | 无需操作，静默生效 |
